...
import { setTitle, setDescription, setBody, addTag, removeTag, setTagList } from "../store/article/createAndEdit/createArticleReducer";
...
const EditPost = () => {
    ...
    const { title, description, body, tagList } = useSelector((state) => state.createArticle);
    ...
    const onSubmit = async (e) => {
        ...
        const result = await dispatch(editArticle({
            slug,
            title,
            description,
            body,
    ...
    return (
        loading ? (
            <Flex align="center" gap="middle" justify="center" style={{ marginTop: '100px' }}>
                ...
                <div className="edit-post__info">
                    <div className="edit-post__info-title">
                        ...
                        <Controller
                            name="title"
                            control={control}
                            rules={{ validate: value => (value && value.trim() !== '') || 'Title text is required' }}
                            render={({ field }) => (
                                <input
                                    {...field}
                                    placeholder="Title"
                    ...
                    <div className="edit-post__short-description">
                        ...
                        <Controller
                            name="description"
                            control={control}
                            rules={{ validate: value => (value && value.trim() !== '') || 'Description text is required' }}
                            render={({ field }) => (
                                <textarea
                                    {...field}
                                    placeholder="Title"
                    ...
                    <div className="edit-post__text">
                        ...
                        <Controller
                            name="body"
                            control={control}
                            rules={{ validate: value => (value && value.trim() !== '') || 'Body text is required' }}
                            render={({ field }) => (
                                <textarea
                                    {...field}
                                    placeholder="Text"
                        ...
                        <div className="edit-post__tags-list">
                            {tagList.map((tag, index) => (
                                <div key={index} className="edit-post__tags-item">
                                    ...
                                    />
                                    <button className='edit-post__tags-button-delete' type="button" onClick={() => handleRemoveTag(index)}>Delete</button>
                                    {index === tagList.length - 1 && (
                                        <button className='edit-post__tags-button-add' type="button" onClick={() => handleAddTag()}>Add tag</button>
                                    )}
                                </div>
                            ))}
                        </div>
                    </div>
                    <button type="submit" className="edit-post-send-button">Send</button>
                </div>
            </form >
        )
    )
